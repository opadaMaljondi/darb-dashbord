...
class FedaPayController extends Controller
{
    ...
    public function fedapay(ValidatorRequest $request)
    {
        ...
        $currency = $user->countryDetail->currency_code ?? "XOF";
        $currency = "XOF";

        return view('fedapay.fedapay', compact('amount', 'payment_for', 'currency', 'user_id','request_id','plan_id'));
    }
    ...
    public function fedapayCheckout(ValidatorRequest $request)
    {
        ...
        $response = Transaction::create([
            ...
            "amount" => 100,
            "callback_url" => route('fedapay.checkout.success') . '?payment_for=' . urlencode($payment_for) . '&currency=' . urlencode($currency) . '&amount=' . urlencode($amount) . '&user_id=' . urlencode($user_id). '&request_id=' . urlencode($request_id). '&plan_id=' . urlencode($plan_id),
            "currency" => [
                "iso" => $currency
            ]
        ]);
